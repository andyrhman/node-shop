...
export const Login = async (req: Request, res: Response) => {
  ...
  const user: User = await myPrisma.user.findFirst({
    where: {
      OR: [
        { email: body.email?.toLowerCase() },
        { username: body.username?.toLowerCase() }
...
export const UpdateInfo = async (req: Request, res: Response) => {
  ...
  if (body.email && body.email !== existingUser.email) {
    const existingUserByEmail = await myPrisma.user.findUnique({
  ...
  if (body.username && body.username !== existingUser.username) {
    const existingUserByUsername = await myPrisma.user.findUnique({
  ...
  const { password, ...updated } = await myPrisma.user.update({ where: { id: user.id }, data: { ...existingUser } });
...
export const UpdatePassword = async (req: Request, res: Response) => {
  ...
  const { password, ...updated } = await myPrisma.user.update({
    where: { id: user.id },
...
export const VerifyAccount = async (req: Request, res: Response) => {
  ...
  await myPrisma.user.update({ where: { id: user.id }, data: { is_verified: true } });
...
export const googleAuth = async (req: Request, res: Response) => {
  try {
    ...
    let user = await myPrisma.user.findUnique({ where: { email: googleUser.email } });
    ...
    if (!user) {
      ...
      user = await myPrisma.user.create({
        data: {
    ...
    const jwt = await new Promise((resolve, reject) => {
      FB.api(
        ...
        async (res: any) => {
          ...
          let user = await myPrisma.user.findUnique({ where: { email: res.email } });
